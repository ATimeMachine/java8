...
public class M2M {
	...
	private static void search(final int page) throws Exception {
		...
		RequestConfig requestConfig = RequestConfig.custom().setCookieSpec(CookieSpecs.STANDARD).setSocketTimeout(10000).setConnectTimeout(10000).build();
		...
		while(it.hasNext()) {
			...
			if (!item.getBooleanValue("inSession")) {
				threadPool.execute(new Runnable() {
					@Override
					public void run() {
						send(item.getLongValue("simId"), item.getString("iccid"), page);
					}
				});
			}
	...
	private static void send(long id, String iccid, int page) {
		try {
			SSLContext ctx = SSLContexts.custom().useProtocol("TLSv1.2").build();
			CloseableHttpClient httpclient = HttpClientBuilder.create().setSslcontext(ctx).build();
			...
			HttpEntity entity = response.getEntity();
			String result = EntityUtils.toString(entity, "UTF-8");
			System.out.println("第"+page+"页，SIMID："+id+"，ICCID："+iccid+"，取消位置发送响应：" + result);
		} catch (Exception e) {
			System.out.println("第"+page+"页，SIMID："+id+"，ICCID："+iccid+"，取消位置发送异常：" + e.getMessage());
		}
	}
}
