...
public class HttpClientUtils {
	...
	// private static final String WXDEV_SERVER_URL =
	// "http://wxtest.weilian.cn/b2b.php?m=b2b&c=data&a=";

	// private static final String WXDEV_SERVER_URL =
	// "http://wxdev.weilian.cn/b2b.php?m=b2b&c=data&a=";

	private static final int CONNECT_TIME_OUT = 30000;

	private static final int STATUS_CODE_FAIL = 200;
	...
	public static String postJson(String method, String jsonData, String url) throws IOException {
		...
		try {
			...
			if (!"".equals(url)) {
				...
				RequestConfig defaultRequestConfig = RequestConfig.custom().setSocketTimeout(CONNECT_TIME_OUT)
						.setConnectTimeout(CONNECT_TIME_OUT).setConnectionRequestTimeout(CONNECT_TIME_OUT)
				...
				if (STATUS_CODE_FAIL != response.getStatusLine().getStatusCode()) {// 失败
	...
	private static HttpPost createHttpPost(String url) {
		 ...
		 * RequestConfig.custom().setSocketTimeout(baseConfig.getSocketTimeout())
		 * .setConnectTimeout(baseConfig.getConnectTimeout()).
		 * setConnectionRequestTimeout(baseConfig.getConnectTimeout())
		 * .setStaleConnectionCheckEnabled(true).build();
		 */
		RequestConfig defaultRequestConfig = RequestConfig.custom().setSocketTimeout(300000).setConnectTimeout(300000)
				.setConnectionRequestTimeout(300000).setStaleConnectionCheckEnabled(true).build();
		httppost.setConfig(defaultRequestConfig);
		return httppost;
	}
